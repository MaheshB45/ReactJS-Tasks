...
const Task = ({ id, title, completed, onCompleteTask, onEditTask, onDeleteTask }) => {
  ...
  return (
    <li className='lg:p-2 flex flex-col lg:flex-row lg:gap-x-2 bg-blue-ribbon-600 dark:bg-blue-ribbon-700 rounded'>
      {
        edit
          ...
          : (
            <>
              <div className='lg:grow h-8 flex items-center lg:gap-x-2 overflow-hidden'>
                <span className={`${completedClass} lg:order-2 w-full pl-1 text-white dark:text-gray-200 rounded overflow-hidden overflow-ellipsis`}>{title}</span>
